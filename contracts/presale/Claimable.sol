...
import "../interfaces/IXClaimCondition.sol";
import "../lib/XBitmap.sol";
import "../lib/MerkleProofs.sol";

abstract contract Claimable is IXClaimCondition{
...
struct AllowlistProof {
    ...
    using XBitMaps for XBitMaps.BitMap;
  ...
  if (validMerkleProof && _allowlistProof.maxQuantityInAllowlist > 0) {
        ...
        claimCondition.lastClaimTimestamp[activeConditionId][_claimMsgSender()] = block.timestamp;
        ...
        emit TokensClaimed(activeConditionId, _claimMsgSender(), _receiver, startTokenId, _quantity);
...
function setClaimConditions(ClaimCondition[] calldata _conditions, bool _resetClaimEligibility)
    ...
    function _claimMsgSender() internal virtual returns (address) {
        return msg.sender;
    }
